...
if (isset($_SESSION["username"])) {
    if ($_SESSION["username"] == "admin") { ?>
						...
						<li class="nav-item">
							<a class="nav-link active" aria-current="page" href="addnews.php">News</a>
						</li>
						<li class="nav-item">
...
if (isset($_SESSION["username"])) { ?>
						...
						<li class="nav-item dropdown">
							...
							<ul class="dropdown-menu bg-danger" aria-labelledby="navbarDropdownMenuLink">
								<li><a class="dropdown-item bg-danger text-white fw-bold text-center" href="addnews.php?logout='1'">logout</a></li>
							</ul>
						</li>
						<?php
} else { ?>
...
?>
		...
		<div class="row">
			<div class="col-2"></div>
			<div class="col-8">
				<h1 class="text-center m-2 mb-4 p-1 border border-dark border-3 border-top-0 border-end-0 rounded">Edit News</h1>
			</div>
			<div class="col-2"></div>
		</div>
		<div class="row">
...
if (isset($_POST["submit"]) && !empty($_FILES["file"]["name"])) {
    ...
    if (in_array($type, $allowed)) {
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $path)) {
            $insert = "UPDATE news SET newsheading= '$heading', newscontent='$content', newsimage='$fileName' WHERE id= '$id'";
...
if (isset($_POST["submit"]) && empty($_FILES["file"]["name"])) {
	...
	$content = $_POST["content"];
    $id = $_GET["id"];
            $insert = "UPDATE news SET newsheading= '$heading', newscontent='$content' WHERE id= '$id'";
            $res = mysqli_query($conn, $insert);
            if ($insert) {
                echo "News Edited Successfully";
                echo("<script>location.href = 'addNews.php';</script>");
