			...
			<div class="container-fluid ">
				...
				if (isset($_SESSION["username"])) {
					if ($_SESSION["username"] == "admin") { ?>
						...
						<li class="nav-item">
							<a class="nav-link active" aria-current="page" href="addevent.php">Events</a>
						</li>
						<li class="nav-item">
...
if (isset($_SESSION["username"])) { ?>
						...
						<li class="nav-item dropdown">
							...
							<ul class="dropdown-menu bg-danger" aria-labelledby="navbarDropdownMenuLink">
								<li><a class="dropdown-item bg-danger text-white fw-bold text-center" href="addevent.php?logout='1'">logout</a></li>
							</ul>
						</li>
						<?php
} else { ?>
...
?>
		...
		<div class="row">
			<div class="col-2"></div>
			<div class="col-8">
				<h1 class="text-center m-2 mb-4 p-1 border border-dark border-3 border-top-0 border-end-0 rounded">Edit Event</h1>
			</div>
			<div class="col-2"></div>
		</div>
		<div class="row">
			...
			<div class="col-8">
				<form class="row g-3 m-2 border-dark border-3 rounded px-2" name="form" method="post" action="" enctype="multipart/form-data">
					...
					<div class="col-md-6 mb-3">
						<label for="exampleFormControlInput1" class="form-label fw-bold">Event Heading</label>
						<input type="text" class="form-control" id="exampleFormControlInput1" name="heading" placeholder="Enter Heading" required value="<?php echo $heading ?>">
					</div>
					<div class="col-md-6">
						<label for="formFile" class="form-label fw-bold">Event Heading Image</label>
						<input class="form-control" type="file" id="formFile" name="file">
					</div>
					<div class="col-md-12 mb-3">
						<label for="exampleFormControlTextarea1" class="form-label fw-bold">Event Content</label>
						<textarea class="form-control" id="exampleFormControlTextarea1" rows="5" name="content" placeholder="Enter Content" required><?php echo $content ?></textarea>
					</div>
...
if (isset($_POST["submit"]) && !empty($_FILES["file"]["name"])) {
    ...
    if (in_array($type, $allowed)) {
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $path)) {
            $insert = "UPDATE events SET heading= '$heading', content='$content', headingimage='$fileName' WHERE id= '$id'";
...
if (isset($_POST["submit"]) && empty($_FILES["file"]["name"])) {
	...
	$content = $_POST["content"];
    $id = $_GET["id"];
            $insert = "UPDATE events SET heading= '$heading', content='$content' WHERE id= '$id'";
            $res = mysqli_query($conn, $insert);
            if ($insert) {
                echo "Event Edited Successfully";
                header("location:addevent.php");
